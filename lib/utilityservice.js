...
const getPuppeteerOptions = async function (jobInfo) {
...
const prepareBrowser = async (jobInfo) => {
    if (!browser) {
        ...
        if (!puppeteerOptions.executablePath && !jobInfo.puppeteerInstalled) {
            ...
            try {
                ...
                VlocityUtils.verbose('Browser launched');
            } catch (error) {
...
// use fastq to ensure only one browser is launched at a time
const queue = require('fastq').promise(prepareBrowser, 1)

UtilityService.prototype.getPuppeteerBrowser = async function(jobInfo) {
    await queue.push(jobInfo);
    return browser;
}

UtilityService.prototype.detectDeadBrowser = async function(error) {
    ...
    if (error.message && error.message.includes('Protocol error') && browser) {
        VlocityUtils.error('Browser detected as dead, closing');
        await this.closePuppeteerBrowser();
    }
}

UtilityService.prototype.closePuppeteerBrowser = async function() {
    if (browser) {
        try { 
            await browser.close();
            VlocityUtils.verbose('Browser closed');
         } catch (e) { 
            VlocityUtils.verbose('Browser failed to close', e);
        }
        browser = null;
        authorized = false;
    }
...
function attachPageLog(page) {
    page.logs = [];

    const eat = (type, txt) => {
        const message = `browser ${type}: ${txt}`
        VlocityUtils.verbose(message);
        page.logs.push(message);
    }

    page.on('console', (msg) => eat('console', msg.text()))
    page.on('pageerror', (error) => eat('pageerror', error.message));
    // page.on('response', (response) =>
    //     eat('response', `${response.status()} ${response.url()}`));
    page.on('requestfailed', (request) =>
        eat('requestfailed', `${request.failure().errorText} ${request.url()}`));
}

// authorize only once
let authorized = false;

UtilityService.prototype.getPuppeteerPage = async function(jobInfo) {

    const browser = await this.getPuppeteerBrowser(jobInfo);
    ...
    if(!authorized) {
        ...
        await Promise.all([
            ...
            page.goto(loginURl, { timeout, waitUntil: ['load', 'networkidle2'] }),
        ]);

        const logo = '.slds-global-header__logo'
...
UtilityService.prototype.savePuppeteerScreenLog = async function(jobInfo, page, name='') {
    if(browser && page) {
        const browserLogDir =  path.join(jobInfo.tempFolder, 'browser');
        await fs.ensureDir(browserLogDir);
        name = name.replace(/[^a-zA-Z0-9]/g, '_');
        const fileSuffix = `${Date.now()}-${name}`
        const screenshotFile = path.join(browserLogDir, fileSuffix + '.png');
        const logFile = path.join(browserLogDir, fileSuffix + '.txt');
        await page.screenshot({ path: screenshotFile });
        await fs.writeFile(logFile, page.logs.join('\n'));
    }
}
